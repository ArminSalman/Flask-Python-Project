...
class TeacherRegisterForm(Form):
    ...
    username = StringField("Kullanıcı Adı",[validators.DataRequired(message="Kullanıcı adı giriniz"),validators.Length(min=3,max=12,message="Kullanıcı adınız en az 3 en fazla 15 karakter olmalıdır")])
...
def teacherRegister():
    ...
    if request.method == "POST" and form.validate():
        ...
        try:
            ...
            query = "Insert into Accounts(name,familyname,username,email,branch,password,teacher,cityCode,teacherVerify,note,admin,work) Values (?,?,?,?,?,?,?,?,?,?,?,?) "
            userList = (name,familyname,username,email,branch,secretpassword,teacher,cityCode,teacherVerify,note,admin,work)
...
def normalRegister():
    ...
    if request.method == "POST" and form.validate():
        ...
        try:
            dataBase = sql.connect("ogretmenim.com.db")
            cursor = dataBase.cursor()
            query = "Insert into Accounts(name,familyname,username,email,password,teacher,teacherVerify,note,admin,work) Values (?,?,?,?,?,?,?,?,?,?) "
            userList = (name,familyname,username,email,secretpassword,teacher,teacherVerify,note,admin,work)
            cursor.execute(query,userList)
            dataBase.commit()
            cursor.close()
            dataBase.close()
    ...
    if request.method == "POST" and form.validate():
        ...
        if data is not None:
            ...
            if sha256_crypt.verify(password,userPassword):
                ...
                session["name"] = name
                session["work"] = work
                session["profilephotoname"] = profilephotoname
                if admin == 1:
                    session["admin"] = True
                if admin == 0:
                    session["admin"] = False
...
def myProfile():
    ...
    email = session["email"]
    branch = None
    if session["teacher"] and session["teacherVerify"]:
        branch = session["branch"]

    database = sql.connect("ogretmenim.com.db")
    cursor = database.cursor()
    queryUser = "Select note,profilephotoname from Accounts where id = ?"
    cursor.execute(queryUser,(userid,))
    data = cursor.fetchone()
    note = data[0]
    profilePhotoName = data[1]
    user = {'userid':userid,'name':name,'familyname':familyname,'username':username,'email':email,'branch':branch,'note':note,'profilePhotoName':profilePhotoName}
...
def profile(username):
    database = sql.connect("ogretmenim.com.db")
    cursor = database.cursor()
    queryUser = "Select username,teacher,teacherVerify,branch,note,work,profilephotoname from Accounts where username = ?"
    cursor.execute(queryUser,(username,))
    data = cursor.fetchone()
    user = {
        'username':username,
        'teacher':data[1],
        'teacherVerify':data[2],
        'branch':data[3],
        'note':data[4],
        'work':data[5],
        'profilePhotoName':data[6]
    }
...
@app.route('/change-profile-photo',methods=["GET","POST"])
def changeProfilePhoto():
    if request.method == "POST":
        ...
        queryPhoto = "Update Accounts set profilephotoname = ? where id = ?"
        cursor.execute(queryPhoto,(filename,session["userid"]))
        queryCommentsPhoto = "Update Comments set profilephotoname = ? where author = ?"
        cursor.execute(queryCommentsPhoto,(filename,session["username"]))
        database.commit()
        cursor.close()
        database.close()
        ...
        return render_template("changeprofilephoto.html")
...
def addComment(questionID):
    ...
    author = session["username"]
    profilephotoname = session["profilephotoname"]
    time = datetime.now()
    time = str(time)
    time = time.split(" ")
    date = time[0]
    ...
    dataBase = sql.connect("ogretmenim.com.db")
    cursor = dataBase.cursor()
    query = "Insert into Comments(content,author,questionID,likecounter,date,profilephotoname) values(?,?,?,?,?,?)"
    cursor.execute(query,(comment,author,questionID,likeCounter,date,profilephotoname))
    query2 = "Select note from Accounts where id = ?"
    cursor.execute(query2,(session["userid"],))
    dataNote = cursor.fetchone()
    note = dataNote[0]
...
def ranking():
    ...
    if rankSelect != None:
        ...
        #User Rank
        if session:
            if session["teacher"] == 0 and rankSelect == "Normal":
                queryInfo = "Select username,teacher,branch,teacherVerify,note,admin from Accounts where id = ?"
                cursor.execute(queryInfo,(session["userid"],))
                info = cursor.fetchone()
                i = 1
                match = True
                while match:
                    for a in range(0,i):
                        if users[a] == info:
                            ...
                            i += 1
            elif session["teacherVerify"] == 1 and rankSelect == "Teacher":
                queryInfo = "Select username,teacher,branch,teacherVerify,note,admin from Accounts where id = ?"
                cursor.execute(queryInfo,(session["userid"],))
                info = cursor.fetchone()
                i = 1
    ...
    return render_template("ranking.html",users=users,rankSelect=rankSelect,lenUsers=lenUsers,userInfo=userInfo,rank=rank)
